...
public class FireSlimeAI : MonoBehaviour
{
    ...
    [SerializeField] private float evolvedScaleMultiplier = 1.18f;
    ...
    {
        if (state != FireSlimeState.Bounce || frameCount <= 1)
        {
            if (!isEvolving)
            {
    ...
    private void TickEvolution()
    {
        ...
        transform.localScale = originalBaseScale * Mathf.Lerp(1f, evolvedScaleMultiplier * 1.2f, pulse);
